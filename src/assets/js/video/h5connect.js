...
function(global) {
	...
	Base.prototype.__get_class_info__ = function() {
	...
	}, Base.prototype.loop = Base.loop = function() {}, Base.extend = function(protoProps, staticProps) {
		...
		return Fn.prototype = sup.prototype, constructor.prototype = new Fn, util.merge(constructor.prototype, protoProps), util.merge(constructor, sup, !0), util.merge(constructor, staticProps), util.merge(constructor, {
	...
	return Base.prototype.on = function(ctx, event, fn) {
		...
		for (var i = 0, j = ary.length; j > i; i++) this.on(ctx, ary[i].event, ary[i].fn)
		...
		util.each(list, function(handler, index) {
			return handler === fn ? (find = index, !1) : void 0
		}), -1 !== find && list.splice(find, 1)
		...
		request = function(address, cbName, cb) {
			return function() {
				global[cbName] = function(data) {
					...
					"undefined" != typeof global[cbName] && (delete global[cbName], cb({
						retcode: 1e4,
						errmsg: "请求超时"
					}))
	...
	return function(global) {
		...
		var _error = [],
			...
			_isEmpty = function(obj) {
				return null === obj ? !0 : _isOBJByType(obj, "Number") ? !1 : !obj
			},
			...
			_send = function(isReoprtNow) {
				if (_config.report) {
					for (; _error.length;) {
						...
						if (_isOBJByType(_config.ignore, "Array")) for (var i = 0, l = _config.ignore.length; l > i; i++) {
			...
			report = {
				push: function(msg) {
					return Math.random() >= _config.random ? report : (_error.push(_isOBJ(msg) ? _processError(msg) : {
						msg: msg
					}), _send(), report)
				},
				report: function(msg) {
...
}), qcVideo("codeReport", function(api) {
	...
	var now = function() {
		...
		REPORT_URL = "//c.isdspeed.qq.com/code.cgi",
		REPORT_URL_HTTPS = "//huatuocode.huatuo.qq.com/code.cgi",
		url = "",
		cgiStartPoint = 0,
		cgiEndPoint = 0,
		from = "";
	return {
		...
		reportEnd: function(type, code) {
			cgiEndPoint = now(), url = url.substring(url.indexOf(":") + 3, url.indexOf("?")), report(url.substring(0, url.indexOf("/")), url.substring(url.indexOf("/") + 1, url.length) + "?from=" + from, type, code, cgiEndPoint - cgiStartPoint)
		},
	...
	var util = {
		...
		each: function(opt, cb) {
			var i, j, key = 0;
			if (this.isArray(opt)) for (i = 0, j = opt.length; j > i && !1 !== cb.call(opt[i], opt[i], i); i++);
			else if (this.isPlainObject(opt)) for (key in opt) if (!1 === cb.call(opt[key], opt[key], key)) break
		}
	},
	...
	function(obj) {
		...
		return hours = hours > 0 ? hours + ":" : "", minutes = minutes > 0 ? minutes + ":" : hours > 0 ? "00:" : "", sec = sec > 0 ? sec + "" : hours.length > 0 || minutes.length > 0 ? "00" : "00:00:00", hours = 2 == hours.length ? "0" + hours : hours, minutes = 2 == minutes.length ? "0" + minutes : minutes, sec = 1 == sec.length ? "0" + sec : sec, hours + minutes + sec
	...
	function uuid(len, radix) {
		var i, chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz".split(""),
			uuid = [];
		if (radix = radix || chars.length, len) for (i = 0; len > i; i++) uuid[i] = chars[0 | Math.random() * radix];
		else {
			var r;
			for (uuid[8] = uuid[13] = uuid[18] = uuid[23] = "-", uuid[14] = "4", i = 0; 36 > i; i++) uuid[i] || (r = 0 | 16 * Math.random(), uuid[i] = chars[19 == i ? 3 & r | 8 : r])
		}
		return uuid.join("")
	}
	...
	if (browserApi && !version.IS_MOBILE) for (fullscreenAPI.supportFullScreen = !0, i = 0; i < browserApi.length; i++) fullscreenAPI[specApi[i]] = browserApi[i];
	else fullscreenAPI.supportFullScreen = !1;
	...
	return Base.extend({
		verifyDone: function(data) {
			var me = this;
			util.merge(me.store, data, !0), util.merge(me.store, {
				parameter: me.option
			});
		...
		getFinalVideos: function(o, source) {
			if (o && o.length) {
				for (var map = {}, ai = 0, al = o.length; al > ai; ai++) map[o[ai].split("?")[0]] = o[ai];
				if (source && source.length) {
					for (var st, finalVideos = [], vi = 0, vl = source.length; vl > vi; vi++) st = source[vi].url.split("?")[0], map.hasOwnProperty(st) && (source[vi].url = map[st], finalVideos.push(source[vi]));
					return finalVideos
				}
			}
			return null
		...
		askDoor: function(firstTime, pass) {
			var key, me = this,
				store = me.store,
				address = constants.SERVER_API + "?interface=Vod_Api_GetPlayInfo&1=1";
			for (key in constants.SERVER_API_PARAMS) store.hasOwnProperty(key) && (address += "&" + key + "=" + store[key]);
			void 0 !== pass && (address += "&pass=" + pass), codeReport.reportStart(address, "h5_vod"), me.loading(!0), api.request(address, function(ret) {
				me.loading();
				var code = ret.retcode + "",
					data = ret.data;
				if (TDBankReporter.pushEvent("connectPlayCgiH5", {
					setting: {
						app_id: me.store.app_id
					}
				}), code != constants.ERROR_CODE.TIME_OUT && code != constants.ERROR_CODE.SCRIPT_ONLOAD && code != constants.SCRIPT_ONERROR ? codeReport.reportEnd("0" == code ? "1" : "3", code) : codeReport.reportEnd(2, code), code == constants.OK_CODE) {
					...
					if (isPwdError && firstTime) me.renderPWDPanel();
					else {
						var errorMsg = constants.ERROR_MSG[code] || "";
						errorMsg && isPwdError || (errorMsg += " Code:(" + code + ")"), me.erTip(errorMsg, isPwdError)
					}
				}
			})
		},
		...
		constructor: function(_$, targetId, opt, eid, link) {
			$ = _$;
			var me = this;
			me.option = opt, me.sdklink = link, me.addStyle(), me.store = util.merge({
				$renderTo: $("#" + targetId),
		...
		erTip: function(msg, pwdEr) {
			pwdEr ? this.$pwd.find(".txt").text(msg).css("visibility", "visible") : this.$error.text(msg).show()
		},
	...
	function rstr_hmac_md5(key, data) {
		var i, hash, bkey = rstr2binl(key),
			ipad = [],
			opad = [];
		for (ipad[15] = opad[15] = void 0, bkey.length > 16 && (bkey = binl_md5(bkey, 8 * key.length)), i = 0; 16 > i; i += 1) ipad[i] = 909522486 ^ bkey[i], opad[i] = 1549556828 ^ bkey[i];
		return hash = binl_md5(ipad.concat(rstr2binl(data)), 512 + 8 * data.length), binl2rstr(binl_md5(opad.concat(hash), 640))
	}
	function rstr2hex(input) {
		var x, i, hex_tab = "0123456789abcdef",
			output = "";
	...
	function matchIfmWidth(opt, ele) {
		...
		var rate = opt.width / opt.height,
			tempW = css.getVisibleWidth(),
			tempH = css.getVisibleHeight();
	...
	function setSuitableWH(opt, ele) {
		var width = opt.width,
			height = opt.height,
			pW = css.getWidth(ele),
			pH = css.getHeight(ele),
			rate = width / height,
			minPix = 4;
		if (minPix > pW && ele.parentNode) for (var pEle = ele.parentNode;;) {
		...
		minPix > pH && pW > minPix && (pH = pW / rate), opt.match_page_width && !matchIfmWidth(opt, ele) || (minPix > pW && (pW = width), minPix > pH && (pH = pW / rate), width > pW && (width = pW, height = width / rate), height > pH && (height = pH, width = height * rate), opt.width = width - 0, opt.height = height - 0)
	}
	var eidUuid = +new Date,
		ableReportJsError = 0;
	return Base.extend({
		className: "Player",
		verifyDone: function(targetId, opt, listener) {
			...
			var ver = version.getVodPriority(inWhiteAppId),
				eid = getEid(),
				flashAble = version.getFlashAble(),
			...
			if ("h5" == ver) qcVideo.use("$", function($) {
			...
			}), TDBankReporter.pushEvent("connectInitH5", {
				setting: {
					app_id: opt.app_id
				}
			});
				...
				"lowVersion" == flashAble ? ele.innerHTML = constants.UNICODE_WORD.TIP_UPDATE_FLASH + '<a target="_blank" style="color:blue;" href="http://www.macromedia.com/go/getflashplayer">' + constants.UNICODE_WORD.TIP_CLICK_UPDATE_FLASH + "</a>" : (new Swf(targetId, eid, opt, link), TDBankReporter.pushEvent("connectInitFlash", {
					setting: {
						app_id: opt.app_id
					}
				}))
		...
		constructor: function(targetId, opt, listener) {
			if (TDBankReporter.pushEvent("connectInit", {
				setting: {
					app_id: opt.app_id
				}
			}), util.isPlainObject(targetId)) {
				var tmp = opt;
			...
			if (ableReportJsError = 0 === ableReportJsError && opt && opt.app_id in constants.BAD_JS_REPORT_WHITE_APP_IDS ? 1 : -1, 1 === ableReportJsError && BJ_REPORT.init({
				...
				if (video) return video.urls ? this.verifyDone(targetId, opt, listener) : alert("缺少视频地址信息，请补齐urls；");
...
}), qcVideo("SwfJsLink", function(util, JSON, H5, codeReport, vodReporter) {
	var global = window,
		cap = function(str) {
		...
		SwfJsLink = function(id, listeners) {
			...
			listeners && "function" != type ? "object" == type && util.merge(me.listeners, listeners) : me.listeners.playStatus = listeners ||
			function() {}, global[id + "_callback"] = function(cmd) {
				var cmds = cmd.split(":"),
					key = cmds[0];
				...
				"codeReport" == cmd && codeReport.reportParams(data), "vodReport" == cmd && vodReporter.reportParams(data)
			}
		};
	return util.each(["volume", "duration", "currentTime", "clarity", "allClaritys"], function(name) {
		...
		changeVideo: tryIt(function(opt) {
			...
			var args = this.getSwf()[this.source](),
				link = this;
		...
		addBarrage: tryIt(function(ary) {
			return ary && ary.length > 0 ? this.getSwf()[this.barrage](JSON.stringify(ary)) : void 0
		}),
	...
	return _detectOS(), {
		className: "TDBankReporter",
		pushEvent: pushPlayEvent
	}
	...
	return Base.instance({
		...
		render: function(opt) {
			...
			var parameter = opt.parameter || {},
				...
				fileInfo = opt.file_info,
				video = fileInfo.image_video.videoUrls || [],
			...
			$.each(video, function(_, item) {
				...
				0 == def ? item.url.toLowerCase().indexOf(".mp4") > 0 && (sourceIsMp4 = !0) : def > 1 && (hasMoreFormat = !0), store.videos[def] = {
					definition: def,
					name: fileInfo.name || "",
					url: item.url,
					height: item.vheight,
	...
	var volume_width = 49,
		...
		svg_info = {
			play: {
				title: WORD.PLAY,
				method: "svg_bottom_play"
			},
			pause: {
				title: WORD.PAUSE,
				method: "svg_bottom_pause"
			},
			volume: {
				title: WORD.MUTE,
				method: "svg_bottom_volume"
			},
			mute_volume: {
				title: WORD.VOLUME,
				method: "svg_bottom_volume_mute"
			},
			full_volume: {
				title: WORD.VOLUME,
				method: "svg_bottom_volume_full"
			},
			fullscreen: {
				title: WORD.FULL_SCREEN,
				method: "svg_bottom_fullscreen"
			},
			quit_fullscreen: {
				title: WORD.EXIT_FULL_SCREEN,
				method: "svg_bottom_quit_fullscreen"
			}
		};
	return UICom.extend({
		...
		init: function() {
			...
			me.subs = {}, me.$el.find("[sub-component]").each(function() {
				...
				38 == keyCode ? me.volumeDrag && me.fire(FIRE, {
					event: EVENT.UI_SET_VOLUME,
					value: {
						volume: Math.min(1, me.volumeDrag.getPercent() + .1)
					}
				}) : 40 == keyCode ? me.volumeDrag && me.fire(FIRE, {
					event: EVENT.UI_SET_VOLUME,
					value: {
						volume: Math.max(0, me.volumeDrag.getPercent() - .1)
					}
				}) : 39 == keyCode ? me.fire(FIRE, {
					event: EVENT.UI_PLUS_PLAY,
					value: {}
				}) : 37 == keyCode && me.fire(FIRE, {
					event: EVENT.UI_MINUS_PLAY,
					value: {}
				})
		...
		_setSimulationPercent: function(percent) {
			...
			setTimeout(function() {
				me.fire(FIRE, {
					event: EVENT.UI_SIMULATION_POSITION,
					value: {
						percent: percent
					}
				})
		...
		setTime: function(obj) {
			var subs = this.subs;
			obj.currentTime !== undefined && this.duration && (subs.time_played.html(util.convertTime(obj.currentTime)), this._setProgress({
				pPercent: util.fix2(obj.currentTime / this.duration * 100)
			})), obj.duration !== undefined && (this.duration = obj.duration, subs.time_duration.html(" / " + util.convertTime(obj.duration))), obj.loaded !== undefined && this.duration && this._setProgress({
				lPercent: util.fix2(obj.loaded / this.duration * 100)
			})
		},
		...
		on_click_setting: function() {
			this.fire(FIRE, {
				event: EVENT.UI_OPEN_SETTING,
				value: {}
			})
		},
		on_click_play_btn: function($dom) {
			this.fire(FIRE, {
				event: EVENT["play" == $dom.attr("now") ? "UI_PLAY" : "UI_PAUSE"],
				value: {}
			})
		},
	...
	return UICom.extend({
		...
		_fixButtonSize: function() {
			var me = this,
				offset = me.store.getMainOffset(),
				max = version.IS_MOBILE ? 2e3 : 75,
				size = Math.min(.3 * Math.min(offset.width, offset.height), max) / 2;
			me.subs.play.width(2 * size).height(2 * size).css({
				margin: "-" + size + "px 0 0 -" + size + "px"
			})
		},
		on_click_play: function() {
			this.fire(constants.FIRE, {
				event: constants.EVENT.UI_PLAY,
				value: {}
			})
		}
	})
		...
		_setLogo: function(obj) {
			...
			if (obj && obj.url) {
				var href = obj.redirect && obj.redirect.length > 10 ? obj.redirect : "javascript:void(0);",
					...
					loc = obj.location = 0 | obj.location;
	...
	return UICom.extend({
		...
		_setPoster: function(obj) {
			if (obj && obj.url) {
				var me = this,
					$el = me.$el,
					hasRedirect = obj.redirect && obj.redirect.length > 7,
	...
	return UICom.extend({
		...
		init: function() {
			...
			w > 0 && 500 > w && (this.FULL_VIEW = !0);
			var data = [],
				all = this.store.getUniqueNoDefinition();
			for (var key in all) data.push({
				sdk_no: all[key].resolutionNameNo,
				sdk_name: all[key].resolutionName
			});
			var tplObj = qcVideo.get("MediaPlayer_tpl");
			this.$el.append(tplObj.setting_definition({
				name: "更换清晰度",
				values: data
			}))
		},
		show: function(obj) {
			var me = this,
				on = "setting-definition-value-curr",
				off = "setting-definition-value";
	...
	return Base.extend({
		...
		catchControlStatusChange: Base.loop,
		live: function(match, fn) {
			var me = this,
				get = function(ctx, hand) {
					return function(e) {
	...
	var Drag = function($el, $papa) {
			var me = this;
			me.$papa = $papa, me.$el = $el, me.maxWidth = 0, me._able = !0, me[Drag.EVENT_DRAG_START] = me[Drag.EVENT_DRAG_STOP] = me[Drag.EVENT_DRAG_ING] = function() {}, me.$el.on("dragstart", function(e) {
	...
	return Base.extend({
		...
		ableToPlay: function(t) {
			var me = this,
				stop_time = me.store.getCustomization("stop_time");
		...
		constructor: function(info) {
			...
			var store = me.store = new PlayerStore(info),
				$container = setting.$renderTo.html(MediaPlayer_tpl.main({
					width: setting.width,
				...
				ableRemember = store.isCustomization("remember");
			status.setRememberKey(video.url), status.enableRemember(ableRemember), status.set_duration(store.getDuration()), me.system.fileType === constants.HLS && (ableRemember = !1, store.setCustomization("disable_drag", !0)), store.store.mainId = mid, me.control = new UiControl(store, status, $container), me.control.enableDrag(!store.isCustomization("disable_drag")), me.control.enableFull(!store.isCustomization("disable_full_screen")), me.system.setUrl(video.url), me.system.status.setRunningStatus("wait"), status.setSDKStatus("suspended"), me.batchOn(status, [{
				...
				event: EVENT.OS_TIME_UPDATE,
				fn: function() {
					var played = status.get_played();
					me.control.setTime({
						currentTime: played
					}), me.ableToPlay(played) || system.pause()
				}
			}, {
				event: EVENT.OS_BLOCK,
				fn: function(e) {
					status.setRunningStatus("error"), me.control.setError(Error.NET_ERROR)
				}
			}, {
				event: EVENT.OS_PLAYER_END,
				fn: function(e) {
					playEnd()
				}
			}]);
			...
			version.IS_MOBILE || $container.on(me.tapEvent, function(e) {
				...
				event: EVENT.UI_FULL_SCREEN,
				fn: function(e) {
					me.system.setFullScreen(!0, mid)
				}
			}, {
				event: EVENT.UI_QUIT_FULL_SCREEN,
				fn: function(e) {
				...
				event: EVENT.UI_OPEN_SETTING,
				fn: function(e) {
					me.control.openSetting({
						currentDefinitionSdkNo: me.video_data.resolutionNameNo
					})
				}
			}, {
				event: EVENT.UI_CHOSE_DEFINITION,
				fn: function(e) {
			...
			me.batchOn(system, [{
				event: EVENT.OS_LAND_SCAPE_UI,
				fn: function() {}
			}, {
				event: EVENT.OS_PORTRAIT_UI,
				fn: function() {}
			}, {
				event: EVENT.OS_LOADED_META_DATA,
				fn: function() {
				...
				event: EVENT.OS_DURATION_UPDATE,
				fn: function(e) {
		...
		rememberPlay: function(status, system, duration) {
			var lPlayed = status.get_local_played() - 0,
				lVolume = status.get_local_volume() - 0;
			lVolume > 0 && system.volume(lVolume), lPlayed > 0 && (0 == duration || duration > lPlayed + 5) && (this.whenReadyToPlay = !0, this.debug("记忆播放位置:", lPlayed), this.system.setUrl(this.video_data.url), this.system.play())
		},
		...
		sdk_tecGet: function(attr, st) {
			if (this.store) {
				...
				case "allClaritys":
					var key, all = this.store.getUniqueNoDefinition(),
						ret = [];
	...
	return {
		...
		center: function(data) {
			var __p = [],
				...
				i = data.nick,
				dot = i + "spinner-dot " + i + "spinner-dot";
		...
		style_center: function(data) {
			...
			for (var i = 0; 7 > i; i++) _p("            ."), _p(this.__escapeHtml(nick)), _p("spinner-dot-"), _p(this.__escapeHtml(i + 1)), _p(" { -moz-animation-delay: ."), _p(this.__escapeHtml(i + 1)), _p("s; -webkit-animation-delay: ."), _p(this.__escapeHtml(i + 1)), _p("s; animation-delay: ."), _p(this.__escapeHtml(i + 1)), _p("s }");
	...
	return Base.extend({
		...
		inCache: function(second) {
			var loaded = this.get_loaded(),
				start = this.start_duration();
			return this.debug(loaded, start, second), this.loaded > 0 && loaded + 5 >= second && second >= start ? (this.debug("in mem"), !0) : (this.debug("out mem"), !1)
		}
	})
	...
	return Base.extend({
		...
		_getPoster: function(pos) {
			var image_url = this.store.image_url,
				imgs = this.store.imgUrls,
				patch = this.store.patch,
				maxSize = 0,
				maxPosition = 0,
				tempSize = 0;
			if (patch && patch[pos] && patch[pos].url) return patch[pos];
			if (pos == constants.PATCH_LOC.START && image_url && -1 == image_url.indexOf("p.qpic.cn")) return {
				url: image_url,
				type: pos,
				redirect: ""
			};
			if (image_url && imgs && imgs.length > 0 && pos == constants.PATCH_LOC.START) {
				for (var user_choose_url = image_url.substring(0, image_url.lastIndexOf("/")), i = 0, j = imgs.length; j > i; i++) tempSize = (0 | imgs[i].vheight) * (0 | imgs[i].vwidth), tempSize > maxSize && -1 != imgs[i].url.indexOf(user_choose_url) && (maxSize = tempSize, maxPosition = i);
				return {
					url: imgs[maxPosition].url,
					type: pos,
					redirect: ""
				}
			}
		...
		getVideoInfo: function(res, def) {
			var rst, me = this,
				videos = me.store.videos,
				resolutionPriority = constants.RESOLUTION_PRIORITY,
				getRstFromDefPriority = function(res) {
					var arr = constants.DEFINITION_PRIORITY[res];
					return arr && arr.length > 0 ? videos[arr[0]] || videos[arr[1]] || null : void 0
				},
				all = this.getAllDefinition();
			if (me.debug(":get video:resource==" + res + ";definition==" + def), def !== undefined && (rst = videos[0 | def], all[def] || (rst = null)), rst || res === undefined || (rst = getRstFromDefPriority(0 | res)), !rst) for (var i = 0, j = resolutionPriority.length; j > i && !(rst = getRstFromDefPriority(resolutionPriority[i])); i++);
			if (rst) {
	...
	return window.addEventListener && window.addEventListener("onorientationchange" in window ? "orientationchange" : "resize", orientationChange, !1), Base.extend({
		...
		__getBuffer: function() {
			var end = 0;
			try {
				for (var i = 0, j = this.video.buffered.length; j > i; i++) end = 0 | this.video.buffered.end(i)
			} catch (e) {}
			return end
		},
		_status_ary: [],
		_bind: function() {
			var me = this,
				metadataDone = function() {
					me.metadatadone = !0, me.video.duration && me.video.duration > 0 && me.fire(EVENT.OS_DURATION_UPDATE, {
						duration: me.video.duration
					}), me._seek_time > 1 && me.video.currentTime < 1 && (me.video.currentTime = me._seek_time), me._volume && (me.video.volume = this._volume), me._seek_time = 0, me.fire(EVENT.OS_LOADED_META_DATA)
				},
		...
		setUrl: function(src) {
			var me = this,
				...
				type = me.fileType = util.fileType(src);
			...
			me.metadatadone = !1, me.has_call_play = !1, me._seek_time = 0, me.timeTask.start(), me.status.clear(), me.$video && ($.each(EventAry, function(_, event) {
				me.$video.off(event)
			}), me.video.src = "", me._hls && me._hls.detachMedia(me.video), me.callMethod("pause"), me.$video.remove());
			var tplObj = qcVideo.get("MediaPlayer_tpl");
		...
		setFullScreen: function(isFull, targetID) {
			var me = this,
				$main = $("#" + targetID),
				$win = $(window),
				target = isFull ? $main.get(0) : document;
			if (isFull) {
		...
		volume: function(volume) {
			return volume === undefined ? this.video.volume : void(volume > 1 || this.video && (this._volume = this.video.volume = volume, this.status.set_volume(volume), this.fire(EVENT.OS_VOLUME_CHANGE, {
				volume: volume
			})))
		}
	})
}), qcVideo("UiControl", function($, Base, util, constants, version) {
	var MODE, id = "qvideo_control_" + +new Date + "_",
		uid = 1,
		bottomLeft = 10,
		UiControl = Base.extend({
			className: "UiControl",
			destroy: function() {
				var me = this;
			...
			constructor: function(store, status, $renderTo) {
				...
				$renderTo.append(tpl.controller({
					...
					controller: {
						left: bottomLeft,
						width: width - 2 * bottomLeft - 2,
						id: id + uid
					},
					WORD: constants.UNICODE_WORD
				})), $renderTo.find("[component]").each(function() {
					var $me = $(this),
						component = util.capitalize($me.attr("component")),
						UICom = qcVideo.get(component);
					me.children[component] = new UICom(store, status, $me), me.on(me.children[component], constants.FIRE, function(obj) {
						...
						if (this.enable()) {
							var $dom = $(dom),
								method = "on_click_" + $dom.attr("sub-component");
				...
				}), me.setViewMode(height), this.setWait(), this.setTime({
					duration: me.store.getDuration()
				}), this.setResize(), me.store.isCustomization("hide_h5_setting") && me.$el.find('[sub-component="setting"]').hide()
			},
			setViewMode: function(height) {
				var me = this,
					sourceH = 36,
					$settingEl = this.children.Setting.$el;
				if (version.IS_MOBILE && height > 0) {
					$settingEl.show();
					var rate = sourceH / height,
						askRate = .25,
						zoom = 1;
					rate != askRate && (zoom = height * askRate / sourceH), this.children.Bottom_container.$el.css("zoom", zoom), setTimeout(function() {
						var h = $settingEl.height(),
							ch = me.$el.height() || 1,
							rate = h / ch,
							size = "1rem";.3 > rate ? size = "2rem" : .5 > rate ? size = "1.5rem" : rate > .9 && (size = "0.5rem"), me.log(h, height, ch, rate, size), $settingEl.css("font-size", size).height("100%").hide()
					}, 300)
			...
			setError: function(msg) {
				this.eachChild(function(son) {
					son.catchControlStatusChange(MODE.ERROR, {
						msg: msg
					})
				})
			},
			...
			setResize: function() {
				...
				this.eachChild(function(son) {
					son.catchControlStatusChange(MODE.RESIZE, {
						offset: offset
					})
				})
			},
...
}); /*  |xGv00|30bc4d5d46af90bdcbca2f8537331b0f */