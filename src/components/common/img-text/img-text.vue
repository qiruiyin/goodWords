...
<template>
	<div :class="['img-text', {'img-text-check': imgTextData.hasCheck}]" @click="setCheck">
		...
		<img :src="imgTextData.imgSmallPath" alt="图片">
		<div class="img-text-content">
			<p class="title">{{ imgTextData.title }}</p>
			<p class="version" v-if="imgTextData.versionName">{{ imgTextData.versionName }}</p>
			<p class="price" v-if="imgTextData.price">
				{{ imgTextData.price }}
				<span v-if="imgTextData.num">&nbsp;{{ imgTextData.num }}集</span>
			</p>
		</div>
	</div>
</template>
